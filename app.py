    ...
    if df2.shape[0] >= 2 and df2.shape[1] >= 2:
        ...
        for r0 in prov_rows[:3]:
            if r0 + 1 in df2.index:
                ...
                if tag in {"score", "ציון", "שירות", "ציון שירות"}:
                    out = {}
...
def _render_reco_card(r: Dict, title: str, badge: str = "", primary: bool = False, manager_names: Optional[List[str]] = None):
    ...
    mgr_html  = "".join(f"<span class='manager-chip'>{_esc(m)}: {_esc(f'{p:.0f}%')}</span>" for m,p in mgr_break)
    ...
    cls = "alt-card " + ("alt-primary" if primary else "alt-secondary")
    badge_html = f"<div class='alt-badge'>{_esc(badge)}</div>" if (primary and badge) else ""
    sub_html = "<div class='alt-sub'>שקלול: דיוק 45% · שירות 40% · אלפא/שארפ 15%</div>" if primary else ""
    ...
    <div class="{cls}">
      ...
      <h3>{_esc(title)}</h3>
      {sub_html}
      <div class="alt-adv">{_esc(r.get('מנהלים',''))}</div>
      <div style="margin: 6px 0 10px;">{mgr_html}</div>
      {funds_html}
...
with tab2:
    ...
    if res is None:
        ...
        if not recs:
            st.warning("אין מספיק פתרונות להצגת המלצות. נסה להריץ חיפוש מחדש.")
        else:
            ...
            for key, row, title in [
                ("weighted", weighted, "ההצעה המשוקללת העדיפה (דיוק · אלפא · שירות)"),
                ("accurate", accurate, "ההצעה המדויקת ביותר"),
                ("sharpe",   sharpe,   "ההצעה עם השארפ הטוב ביותר"),
            ]:
                row = dict(row)
                row["חלופה"] = title
                row["weights_items"] = _weights_items(row.get("weights"), row.get("קופות",""), row.get("מסלולים",""))
            ...
            _render_reco_card(weighted, "ההצעה המשוקללת העדיפה", badge="מומלץ", primary=True, manager_names=manager_names)
            st.caption("הצעה זו מאזנת בין **דיוק ליעדים**, **אלפא/שארפ** ו-**שירות** — ולכן היא ברירת המחדל המומלצת לבחירה.")

            # Secondary recommendations (side-by-side, less prominent)
            c1, c2 = st.columns(2)
            with c1:
                _render_reco_card(accurate, "ההצעה המדויקת ביותר", primary=False, manager_names=manager_names)
            with c2:
                _render_reco_card(sharpe, "ההצעה עם השארפ הטוב ביותר", primary=False, manager_names=manager_names)
